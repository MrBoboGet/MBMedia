...
void logging(const char* string,const char* string2)
{
    std::cout << string << std::endl;
}
...
int encode_video(StreamingContext* decoder, StreamingContext* encoder, AVFrame* input_frame) {
    ...
    while (response >= 0) {
        ...
        if (response != 0) { logging("Error %d while receiving packet from decoder: %s"); return -1; }
    }
...
int transcode_audio(StreamingContext* decoder, StreamingContext* encoder, AVPacket* input_packet, AVFrame* input_frame) {
    ...
    if (response < 0) { logging("Error while sending packet to decoder: %s"); return response; }
